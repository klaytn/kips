...
-   IPublicDelegation: an interface that a public delegation must implement to be compatible with CnStakingV3.
...
IPublicDelegation must be implemented in `PublicDelegation` to be fully compatible with CnStakingV3:
...
interface IPublicStakedKlay is IPublicDelegation {
    /* ========== STRUCT ========== */
    ...
    function stakeFor(address _recipient) external payable; // Defined in IPublicDelegation
    ...
    function reward() external view returns (uint256); // Defined in IPublicDelegation
